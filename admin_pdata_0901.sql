...
create table p_event(
    ...
    mt20id	    VARCHAR2(12), --pk, not null	12	공연ID +
    prfnm	      VARCHAR2(100), --	공연명 +
    prfpdfrom	  VARCHAR2(10), --	공연시작일 +
    prfpdto	    VARCHAR2(10), --	공연종료일 +
    fcltynm	    VARCHAR2(60), --	공연시설명(공연장명) +
    poster	    VARCHAR2(100), --	포스터이미지경로
    genrenm	    VARCHAR2(10), --	공연 장르명 +
    prfstate	  VARCHAR2(6), --	공연상태 +
    openrun	    VARCHAR2(1), --	오픈런
    mt10id	    VARCHAR2(10), --	공연시설ID 
    prfcast	    VARCHAR2(100), --	공연출연진 +
    prfcrew	    VARCHAR2(30), --	공연제작진 +
    prfruntime	VARCHAR2(10), --	공연 런타임 +
    prfage	    VARCHAR2(12), --	공연 관람 연령 +
    entrpsnm	  VARCHAR2(20),	--제작사 +
    pcseguidance	NUMBER(6),	--티켓가격 +
    sty	        CLOB,		    -- 줄거리
    styurl	    VARCHAR2(100), --	소개이미지1
    styurl	    VARCHAR2(100), --	소개이미지2
    styurl	    VARCHAR2(100), --	소개이미지3
    styurl	    VARCHAR2(100), --	소개이미지4
    dtguidance	VARCHAR2(60) --	공연시간 +
);
...
insert into p_event (mt20id,prfnm,prfpdfrom,prfpdto,fcltynm,
          genrenm,prfstate,prfcast,prfruntime,prfage,entrpsnm,pcseguidance,dtguidance) values 
          ('PF197254','제2회 리틀 아티스트 콘서트','2022.08.29','2022.08.29','롯데콘서트홀',
          '클래식','공연완료','변지민, 이채은, 전재인, 조별 등','1시간 50분','만 7세 이상','',20,000,'월요일(11:30)');
insert into p_event (mt20id,prfnm,prfpdfrom,prfpdto,fcltynm,
          genrenm,prfstate,prfcast,prfruntime,prfage,entrpsnm,pcseguidance) values 
          ('PF197222','심포니 송 챔버뮤직 시리즈 Ⅲ, Summer Chamber Music Festival','2022.08.29','2022.08.29','일신홀',
          '클래식','공연완료','박병진, 유요한, 장서원, 손혁진 등','1시간 20분','만 7세 이상','',20,000,'월요일(19:30)');
insert into p_event (mt20id,prfnm,prfpdfrom,prfpdto,fcltynm,
          genrenm,prfstate,prfcast,prfruntime,prfage,entrpsnm,pcseguidance) values 
          ('PF197218','SAC on screen, 굿모닝 독도 [제주]','2022.08.26','2022.09.06','제주문예회관',
          '뮤지컬','공연중','','1시간 50분','만 5세 이상','',0,'화요일(15:00)');
insert into p_event (mt20id,prfnm,prfpdfrom,prfpdto,fcltynm,
          genrenm,prfstate,prfcast,prfruntime,prfage,entrpsnm,pcseguidance) values 
          ('PF197207','찾아가는 Let’s DMZ [이천]','2022.08.27','2022.08.27','이천아트홀',
          '복합','공연완료','송창진, 장원진, 문미경, 오빛나 등','1시간 20분','만 7세 이상','',15000,'토요일(15:00)');
insert into p_event (mt20id,prfnm,prfpdfrom,prfpdto,fcltynm,
          genrenm,prfstate,prfcast,prfruntime,prfage,entrpsnm,pcseguidance) values 
          ('PF197197','우리 소리 축제 하하하, 안해본소리 프로젝트','2022.08.26','2022.08.26','대전시립연정국악원',
          '국악','공연완료','','1시간 30분','만 7세 이상','',0,'금요일(19:30)');
insert into p_event (mt20id,prfnm,prfpdfrom,prfpdto,fcltynm,
          genrenm,prfstate,prfcast,prfruntime,prfage,entrpsnm,pcseguidance) values 
          ('PF197161','동,서양의 음악을 핫하고 힙하게!','2022.08.30','2022.08.30','고양아람누리',
          '복합','공연중','이은진, 김도아, 홍아람, 김태훈 등','1시간 20분','만 6세 이상','',0,'화요일(19:30)');
insert into p_event (mt20id,prfnm,prfpdfrom,prfpdto,fcltynm,
          genrenm,prfstate,prfcast,prfruntime,prfage,entrpsnm,pcseguidance) values 
          ('PF197155','님만헤민 게스트하우스','2022.08.26','2022.09.18','소극장 혜화당(구. 까망소극장)',
          '연극','공연중','이재섭, 김자미, 차은진, 조효준 등','1시간 40분','전체 관람가','',20,000,'화요일 ~ 금요일(19:30), 토요일(16:00), 일요일(15:00), HOL(15:00)');
insert into p_event (mt20id,prfnm,prfpdfrom,prfpdto,fcltynm,
          genrenm,prfstate,prfcast,prfruntime,prfage,entrpsnm,pcseguidance) values 
          ('PF197146','유니온 오케스트라 정기연주회','2022.08.28','2022.08.28','고양어울림누리',
          '클래식','공연완료','강채율, 김민채, 김민희, 김서영 등','2시간','만 3세 이상','',0,'일요일(17:00)');
insert into p_event (mt20id,prfnm,prfpdfrom,prfpdto,fcltynm,
          genrenm,prfstate,prfcast,prfruntime,prfage,entrpsnm,pcseguidance) values 
          ('PF197127','내 모든걸','2022.08.25','2022.11.13','문화예술전용극장CT',
          '연극','공연중','','1시간 40분','만 7세 이상','',40,000,'화요일 ~ 금요일(19:30), 토요일(15:00,18:00), 일요일(14:00), HOL(14:00,17:00)');
insert into p_event (mt20id,prfnm,prfpdfrom,prfpdto,fcltynm,
          genrenm,prfstate,prfcast,prfruntime,prfage,entrpsnm,pcseguidance) values 
          ('PF197126','제36회 별망성 전국학생 음협 콩쿠르 수상자음악회','2022.08.27','2022.08.27','안산문화예술의전당',
          '클래식','공연완료','','','전체 관람가','',0,'토요일(14:00,15:00,16:00)');
          
create table p_facility(
    mt10id	    VARCHAR2(10), --	pk, fk, 공연시설ID
    fcltynm	    VARCHAR2(30), --	fk, 공연시설명
    mt13cnt	    NUMBER(2), --	공연장 수
    fcltychartr	VARCHAR2(10), --	시설특성
    seatscale	  NUMBER(5), --	5	객석 수
    telno	      VARCHAR2(12), --	전화번호
    relateurl	  VARCHAR2(100), --	홈페이지
    adres	      VARCHAR2(60), --	주소
    opende	    NUMBER(4), --	개관연도
    la	        VARCHAR2(10), --	위도
    lo	        VARCHAR2(10) --	경도
...
create table p_festival(
    mt40id	    VARCHAR2(12), --	공연ID mt20id -> mt40id
    prfnm	      VARCHAR2(100), --	공연명
    prfpdfrom	  VARCHAR2(10), --	공연시작일
    prfpdto	    VARCHAR2(10), --	공연종료일
    fcltynm	    VARCHAR2(60), --	공연시설명(공연장명)
...
alter table p_festival add constraint enterprise_mt20id_pk primary key(mt40id);

insert into p_festival(mt40id,prfnm,prfpdfrom,prfpdto,fcltynm,poster,genrenm,prfstate)
